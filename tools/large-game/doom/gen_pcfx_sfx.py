...
# AND this ADPCM bank (anchored at the top of bank A). The bank is NOT free
...
# budget is base + sector-rounded bank_words <= 0x20000, not <= 0x40000.
# Keep the old 238 KB payload cap so the set of included sounds is unchanged.
# Once that bank is built, its actual size sets a sector-aligned base, leaving
# all space below it for the MPCONV sky encoder's automatic quality search.
#
...
# The bank ends at page-1 word 0x20000, with its base derived from its size,
...
# and no sample has to avoid a gap. The payload cap is unchanged (243712).
BANK_END_WORD = 0x20000      # end of page-1 bank A (exclusive)
BANK_BUDGET_BYTES = (BANK_END_WORD - 0x02800) * 2
...
def main():
    ...
    for i, name in enumerate(names):
        ...
        if len(bank) + len(adpcm) > BANK_BUDGET_BYTES:
            metas.append((0, 0, 0))
            continue
        # Relative until the completed bank determines its placement.
        start_word = len(bank) // 2
        bank += adpcm
        metas.append((start_word, len(adpcm) // 2, 63))
        n_encoded += 1

    bank_span = (len(bank) + CD_SECTOR_BYTES - 1) // CD_SECTOR_BYTES * CD_SECTOR_BYTES
    kram_base = BANK_END_WORD - bank_span // 2
    # Sector alignment also satisfies SAL's 256-word start alignment.
    metas = [(kram_base + sw, wc, vol) if wc else (0, 0, 0)
             for sw, wc, vol in metas]
    ...
    print(f'SFX: KRAM base word 0x{kram_base:X}, sector span {bank_span} bytes')
