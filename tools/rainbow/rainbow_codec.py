#!/usr/bin/env python3
"""MPCONV-compatible HuC6271 encoder, derived from Doom PC-FX.\n\nSee PROVENANCE.md. Host-only integer coefficient arithmetic.\n"""
...
def prepare_image(rgb):
    """Transform once; budget search reuses the same coefficients at each scale."""
    rgb = np.asarray(rgb, dtype=np.uint8)
    if rgb.ndim != 3 or rgb.shape[1:] != (256, 3) or not 16 <= rgb.shape[0] <= 496 or rgb.shape[0] % 16:
        raise ValueError("input must be RGB, 256 wide, height 16..496 in multiples of 16")
    yuv = pcfx_yuv(rgb)
    strips = []
    for by in range(0, len(rgb), 16):
        macros = []
        for bx in range(0, 256, 16):
            macro = yuv[by:by + 16, bx:bx + 16]
            if is_null_macroblock(macro):
                macros.append(None)
                continue
            planes = [macro[y:y + 8, x:x + 8, 0] for y, x in ((0, 0), (8, 0), (0, 8), (8, 8))]
            planes += [macro[:, :, c].reshape(8, 2, 8, 2).sum(axis=(1, 3)) >> 2 for c in (1, 2)]
            macros.append([mpconv_fdct(np.asarray(p, dtype=np.int64) - 128) for p in planes])
        strips.append(macros)
    return strips


def encode_frame(path, scale=0):
    """Compatibility API for the imported Doom asset builder."""
    with Image.open(path) as img:
        img = img.convert("RGB")
        if img.size != (256, 240):
            img = img.resize((256, 240), Image.Resampling.LANCZOS)
        return encode_prepared(prepare_image(np.asarray(img)), scale)


def encode_prepared(strips, scale=0, independent_strips=False):
    ...
    for strip_index, macros in enumerate(strips):
        writer = BitWriter()
        dc_y = 0
        dc_u = 0
        dc_v = 0
        null_run = 0
        if independent_strips:
            emitted_scale = None
        for macro in macros:
            if macro is None:
                null_run += 1
            ...
            if emitted_scale != scale:
                ...
                emitted_scale = scale
            predictors = [dc_y, dc_u, dc_v]
            for i, coeff in enumerate(macro):
                channel = 0 if i < 4 else i - 3
                qt = qy if channel == 0 else qc
                magnitude = (np.abs(coeff) + (qt >> 1)) // qt
                quant = np.where(coeff < 0, -magnitude, magnitude).reshape(64)
                predictors[channel] = encode_block(writer, quant, predictors[channel],
                    HUFF_DC_Y if channel == 0 else HUFF_DC_UV,
                    HUFF_AC_Y if channel == 0 else HUFF_AC_UV)
            dc_y, dc_u, dc_v = predictors
...
def analyze_stream(stream, block_count=BLOCK_COUNT, independent_strips=False):
    """Validate physical frame layout; return each stored strip's KRAM span."""
    spans = []
    offset = 0
    for strip in range(block_count):
        first = strip == 0 or independent_strips
        marker = b"\xff\xff" if first else b"\xff\xf8"
    ...
    if offset != len(stream):
        raise ValueError("unexpected data after the last strip")
    return spans
