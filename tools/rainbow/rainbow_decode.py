#!/usr/bin/env python3
"""Strict HuC6271 entropy validator and approximate host image preview.

Entropy follows pcfxemu; the floating IDCT and nearest-neighbour chroma here
are approximations, NOT pixel-exact models of either emulator backend or silicon.
Use emulator captures to check actual playback. See PROVENANCE.md.
...
class Stream:
    ...
    def fetch(self):
        if self.pos >= len(self.data):
            raise ValueError("truncated RAINBOW stream")
    ...
    def init_bits(self, bcount):
        ...
        if bcount < 0 or end > len(self.data):
            raise ValueError("invalid physical entropy length")
        raw = bytearray()
        while self.pos < end:
            b = self.fetch()
            raw.append(b)
            if b == 0xFF:
                if self.pos >= end or self.fetch() != 0:
                    raise ValueError("truncated or unstuffed entropy FF")
        self.buf = int.from_bytes(raw, "big")
        self.nbits = len(raw) * 8

    def get_bits(self, count, peek=False, funnysign=False):
        if count > self.nbits:
            if not peek:
                raise ValueError("entropy exhausted before all 16 macroblocks decoded")
            ret = (self.buf & ((1 << self.nbits) - 1)) << (count - self.nbits)
        else:
    ...
    def skip_bits(self, count):
        if count <= 0 or count > self.nbits:
            if count == 0:
                return
            raise ValueError("truncated entropy code")
        self.nbits -= count
...
class Decoder:
    ...
    def get_dc_y(self):
        ...
        while True:
            ...
            code = lut_sym[raw]
            if not lut_bits[raw]:
                raise ValueError("invalid Y DC Huffman code")
            self.s.skip_bits(lut_bits[raw])
            if code < 0xF:
            ...
            self.rescale(code - 0x10)

    def rescale(self, scale):
        for i in range(64):
            self.qtab[0][i] = min(254, max(1, self.qbase[0][i] * scale >> 2))
            c = (self.qbase[1][i] * scale >> 2) if i else (self.qbase[1][0] >> 2)
            self.qtab[1][i] = min(254, max(1, c))

    def finish_strip(self):
        # Complete trailing rescale controls precede 0..15 zero alignment bits.
        while self.s.nbits >= 9:
            raw = self.s.get_bits(9, peek=True)
            code = DC_Y_LUT[0][raw]
            if code < 0x10:
                break
            self.s.skip_bits(9)
            self.rescale(code - 0x10)
        if self.s.nbits > 15 or self.s.get_bits(self.s.nbits, peek=True) != 0:
            raise ValueError("unexpected entropy after column 15 (or missing alignment)")
    ...
    def get_dc_uv(self):
        ...
        code = lut_sym[raw]
        if not lut_bits[raw]:
            raise ValueError("invalid UV DC Huffman code")
        self.s.skip_bits(lut_bits[raw])
...
def decode_stream(data, height=240):
    ...
    if height < 16 or height % 16:
        raise ValueError("height must be a positive multiple of 16")
    s = Stream(data)
    d = Decoder(s)
    Y = np.zeros((height, 256), dtype=np.int32)
    U = np.full((height // 2, 128), 128, dtype=np.int32)
    V = np.full((height // 2, 128), 128, dtype=np.int32)

    row = 0
    while row * 16 < height:
        if s.pos & 1 or s.fetch() != 0xFF:
            raise ValueError(f"strip {row}: invalid marker or word alignment")
        btype = s.fetch()
        size = (s.fetch() << 8) | s.fetch()
        if size & 1 or size < (132 if btype == 0xFF else 4):
            raise ValueError(f"strip {row}: invalid physical payload size {size}")
        end = s.pos + size
        if data[end - 2:end + 6] != b'\0' * 8:
            raise ValueError(f"strip {row}: missing inner dummy/external guard")
        size -= 2
        if btype == 0xFF:
            for q in range(2):
                for i in range(64):
        ...
        if btype not in (0xF8, 0xFF):
            raise ValueError(f'block {row}: unsupported RLE type {btype:#04x}')
        if row == 0 and btype != 0xFF:
            raise ValueError("first strip must initialize quantization tables")
        ...
        while col < 16:
            ...
            dc_y += v
            if zeroes:
                col += zeroes            # programmed null colour: Y=0, U=V=128
                if col > 16:
                    raise ValueError("null run extends past column 15")
                dc_y = dc_u = dc_v = 0
                continue
            blocks = []
            for k in range(4):
                if k:
                    v, nulls = d.get_dc_y()
                    if nulls:
                        raise ValueError("null run inside a macroblock")
                    dc_y += v
        ...
        row += 1
    if s.pos != len(data):
        raise ValueError("unexpected data after final strip")
